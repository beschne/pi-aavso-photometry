...
- [ ] **Ensemble photometry (`CNAME=ENSEMBLE`).** Insert a dedicated "Comp stars" wizard step between Setup and Photometry. On entry it runs a lightweight PSF discovery fit on all in-frame V-band candidates and presents an annotated table (one row per star): V mag, Δmag from target, in-frame, blended flag, PSF quality, and a pre-ticked recommended column with inline "why not" reasons (saturated, too faint, blended, large Δmag). User can override any row. Check star remains a separate ComboBox, excluded from the ensemble pool. Photometry step uses the confirmed selection; report writes `CNAME=ENSEMBLE`, `CMAG=na`, lists comp AUIDs/mags in NOTES. Zero-point = mean(mag_cat − instMag) across ensemble; MERR = ZP scatter + target PSF error in quadrature.
